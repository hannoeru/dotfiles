...
- **Worktree: switch / create from default branch** (`open`) — fzf over existing worktrees and local branches. `Enter` on a match switches to it; type a new name to create it from worktrunk's default base. Press `Tab` to toggle the picker between worktrees and a searchable list of open pull requests.
