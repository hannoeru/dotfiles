 ...
 *
 * 2. Hook bypass prevention — Blocks `--no-verify` and its `git commit -n`
 *    alias so the agent cannot circumvent git hooks (pre-commit, commit-msg,
 *    etc.). The agent should fix hook failures or ask the human for help.
 */
...
const HOOK_BYPASS_RE = /--no-verify\b|\bgit\s+commit\b(?:(?![;&|\n]).)*\s-n(?=\s|[;&|\n]|$)/;

const BLOCK_REASON =
	"BLOCKED: Git hook bypass flags are not allowed. Git hooks exist for a reason. " +
...
export default function (pi: ExtensionAPI) {
	pi.on("tool_call", (event) => {
		...
		if (HOOK_BYPASS_RE.test(event.input.command)) {
