...
5. Check the repo's contribution guidelines (for example `CONTRIBUTING.md`) for its changelog policy. If contributor PRs must not edit the changelog — the maintainer adds the entry when merging — do not check for or request a changelog entry. Otherwise, review the changelog against the repo's convention.
6. Check whether the project's documentation needs updating — typically the README, `docs/*.md`, and `examples/**/*.md` — which is usually required when existing features have changed or new features have been added.
