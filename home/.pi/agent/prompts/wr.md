...
1. Add or update the changelog entry following the convention the repo already uses (for example a `## [Unreleased]` section in `CHANGELOG.md`, Keep a Changelog, or a per-package changelog in a monorepo). If the repo has no changelog convention, skip this step.
...
5. Check the current git branch. If it is not the repo's default branch (for example `main` or `master`), stop and ask what to do. Do not push from another branch unless I explicitly say so.
6. Push the current branch.
7. If this task is tied to exactly one GitHub issue, explicitly close that issue with reason `completed` after the push so the repo's issue-close automation (for example workflows in `.github/`) runs. This applies to issues only, not PRs.
...
- When working against a branch other than the default branch, skip the changelog.